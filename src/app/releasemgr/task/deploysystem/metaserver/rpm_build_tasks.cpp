   ...
   :AbstractTask(taskmgr, invokeArgs)
{
   ...
   //检查项目文件
   QString projectQbsFile(m_projectDir+"/meta_server.qbs");
   if(!Filesystem::fileExist(projectQbsFile)){
      throw ErrorInfo(QString("项目文件夹中项目文件%1不存在").arg("meta_server.qbs"));
   }
...
{
   return AbstractTask::getAssetDir() + DS+"deploysystem";
}
...
void CopySourceFiles::processSpecFile()
{
   ...
   QByteArray specFileContent(Filesystem::fileGetContents(getAssetDir()+DS+"metaserver_rpm_spec.tpl"));
   ...
   Filesystem::filePutContents(m_rpmSpecDir+DS+"metaserver.spec", specFileContent);
   writeDoneMsg();
}
...
void BuildRpm::exec()
{
   ...
   QString cmd = QString ("rpmbuild -bb --define=\"_topdir %1\" %2/metaserver.spec").arg(m_buildDir, m_rpmSpecDir);
