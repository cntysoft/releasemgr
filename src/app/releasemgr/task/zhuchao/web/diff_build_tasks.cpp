...
{
   m_buildDir = getSysCfgValue("diffBuildDir").toString() + QDir::separator() + "zhuchaoweb_patch_" +  m_from +"_"+ m_to;
}
...
void GenerateDiffMetaInfo::saveDiffMetaInfo(GeneralKeyToListMapType& savedMetaInfo, const GeneralKeyToListMapType &modifiedFiles, const QStringList& needRunCmdProjects)
{
   ...
   if(!modifiedFiles["delete"].isEmpty() || !modifiedFiles["modify"].isEmpty()){
      ...
      savedMetaInfo["modify"] << "Library/ZhuChao/Version.php";
      savedMetaInfo["modify"] << "Library/ZhuChao/DistConst.php";
   }
...
void GenerateDiffMetaInfo::buildAndCopyFiles(const GeneralKeyToListMapType& savedMetaInfo, const QStringList& needRunCmdProjects)
{
   ...
   writeDoneMsg();
   //删除源码库里面跟打包相关的代码
   QDir releasemgrDir(m_buildDir+ds+"Library"+ds+"ZhuChao"+ds+"ReleaseTools");
...
void GenerateDiffMetaInfo::getGitDiffFiles(const QString &projectDir, const QString &from, const QString &to, GeneralKeyToListMapType& modifiedFilesMap, const QStringList &paths, const QStringList &skips)
{
   if(from.isEmpty() && to.isEmpty()){
      modifiedFilesMap["modify"] = QStringList();
      modifiedFilesMap["delete"] = QStringList();
      return;
   }
   QProcess process;
   QStringList args;
   process.setWorkingDirectory(projectDir);
   if(from.isEmpty() && !to.isEmpty()){
      args << "ls-files" << "--with-tree" << to;
   }else if(!from.isEmpty() && to.isEmpty()){
      args << "ls-files" << "--with-tree" << from;
   }else{
   ...
   bool status = process.waitForFinished(-1);
   if(!status || process.exitCode() != 0){
      throw ErrorInfo(process.readAllStandardError());
   }
   QString ret(process.readAll());
...
void GenerateVersionInfo::exec()
{
   ...
   QChar ds = QDir::separator();
   QFile versionTplFile(m_buildDir+ds+"Library"+ds+"ZhuChao"+ds+"Version.php");
   ...
   QFile versionFile(m_buildDir+ds+"Library"+ds+"ZhuChao"+ds+"Version.php");
...
void SetupDistConst::exec()
{
   ...
   QChar ds = QDir::separator();
   QString baseDir(StdDir::getAssetsDir()+ds+"zhuchao"+ds+"distconst");
   ...
   QString targetConstFilename = m_buildDir + ds + "Library" + ds + "ZhuChao" + ds + "DistConst.php";
...
void Compress::exec()
{
   ...
   QString sourceDirName = QString("zhuchaoweb_patch_%1_%2").arg(m_from, m_to);
