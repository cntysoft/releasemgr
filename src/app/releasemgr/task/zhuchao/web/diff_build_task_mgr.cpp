...
#include "task/zhuchao/web/diff_build_tasks.h"
...
{
   writeMsg("开始差异性打包凤凰筑巢WEB平台系统, 打包范围为 : ");
...
{
   QString buildDir = getSysSettings().getValue("diffBuildDir", getModuleName()).toString();
   QString filename(buildDir+QDir::separator()+"zhuchaoweb_patch_"+args["from"]+"_"+args["to"]+".tar.gz\n");
