...
{
   m_buildDir = m_buildDir + QDir::separator() + "zhuchaoweb_" + m_invokeArgs["version"];
}
...
void Compress::exec()
{
   ...
   QString sourceDir = "zhuchaoweb_"+version;
   execGzipCmd(QString("zhuchaoweb_%1.tar.gz").arg(version), sourceDir);
   writeDoneMsg();
}
